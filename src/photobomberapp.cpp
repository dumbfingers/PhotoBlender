...
void PhotoBomberApp::switchCamera()
{
	if (camera->cameraUnit() == CameraUnit::Front){
		qDebug() << "Front camera has already been opened, switch to rear camera";
		// Release the current camera hook
		camera->close();
		// Open the opposite camera
		camera->open(CameraUnit::Rear);
	}
	else if (camera->cameraUnit() == CameraUnit::Rear){
		qDebug() << "Rear camera has already been opened, switch to front camera";
		// Release the current camera hook
		camera->close();
		// Open the opposite camera
		camera->open(CameraUnit::Front);
		...
		qDebug() << "No camera has been opened before, something must be wrong";
//		camera->open(CameraUnit::None);
	}
...
void PhotoBomberApp::onCameraOpened()
{
